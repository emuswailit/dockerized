...
const DrugClasses = (props) => {
  ...
  const [generics, setGenerics] = React.useState(props.generics);
  const [formulations, setFormulations] = React.useState(props.formulations);
  ...
  const [editPreparation, setEditingPreparation] = useState(false);
  ...
  const handleDeleteUser = (user_id) => {
    console.log(user_id);
    props.deletePreparation(user_id);
  };
  ...
  const handleEditBodySystem = (row) => {
    console.log("Preperation to edit: ", row);
    setEditingPreparation(true);
    setSelectedPreparation(row);
  };
  ...
  const handleGenericValues = (setFieldValue, value) => {
    const { id } = value;
    console.log("generic", id);

    setFieldValue("generic", id);
  };

  const handleFormulationValues = (setFieldValue, value) => {
    const { id } = value;
    console.log("formulation", id);
    setFieldValue("formulation", id);
  };
    ...
    <div className={classes.root}>
      <Paper className={classes.paper}>
        ...
        <TableContainer>
          ...
          <Fragment>
            {/* 1. Create preparation dialog */}
            <Dialog
              open={creatingPreparation}
              onClose={handleClose}
                  ...
                  <DialogContent>
                    ...
                    <Formik
                      ...
                      validationSchema={Yup.object().shape({
                        ...
                        // ),
                        generic: Yup.string().required("Select preparation"),
                            ...
                            <div>
                              ...
                              <Box my={2}>
                                <Autocomplete
                                  ...
                                  options={generics}
                                  onChange={(e, value) =>
                                    handleGenericValues(setFieldValue, value)
                                  }
                                  // autoHighlight
                                  // selectOnFocus
                  ...
                  <DialogContent>
                    ...
                    <Formik
                      initialValues={{
                        ...
                        description: selectedPreparation.description,
                        generic: selectedPreparation.generic,
                        formulation: selectedPreparation.formulation,
                      }}
                      onSubmit={(values, { setSubmitting }) => {
                        console.log("at submit for edit", values);
                        props.changePreparation(values, selectedPreparation.id);
                      }}
                      validationSchema={Yup.object().shape({
                        title: Yup.string().required("Title is required"),
                        // description: Yup.string().required(
                            ...
                            <div>
                              ...
                              <Box my={2}>
                                <Autocomplete
                                  ...
                                  defaultValue={{
                                    title:
                                      selectedPreparation.generic_details.title,
                                    id: values.generic,
                                  }}
                                  id="generic-select"
                                  options={generics}
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow hover>
                      ...
                      <TableCell align="left">
                        {row.generic_details.title}
                      </TableCell>
                      <TableCell align="left">
                        {row.formulation_details.title}
                      </TableCell>
